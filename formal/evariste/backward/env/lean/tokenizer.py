...
tokenizer: Optional["LeanTokenizer"] = None


class LeanTokenizer(ABC):
    @abstractmethod
    ...
    @staticmethod
    def build(version: Optional[str]) -> "LeanTokenizer":
        global tokenizer
        if version is not None:
            print(f"Building Lean tokenizer ({version}) ...")
            if version == "bpe_arxiv_lean_utf8_20k":
                tokenizer = LeanTokenizerBPEArxivUTF8(bpe_path="", split_digits=False,)
            elif version == "bpe_arxiv_lean_utf8_20k_single_digits":
                tokenizer = LeanTokenizerBPEArxivUTF8(bpe_path="", split_digits=True,)
            elif version == "bpe_arxiv_lean_utf8_20k_single_digits_no_unk":
                tokenizer = LeanTokenizerBPEArxivUTF8(
                    bpe_path="", split_digits=True, alone_digits=True,
                )
            elif version == "char":
...
class CharLevelTokenizer(LeanTokenizer):
    def encode(self, sentence: str) -> List[str]:
...
class LeanTokenizerBPEArxivUTF8(LeanTokenizer):
    def __init__(self, bpe_path: str, split_digits: bool, alone_digits: bool = False):
        ...
        self.bpe: Optional[yttm.BPE] = None

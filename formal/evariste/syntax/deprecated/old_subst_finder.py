...
# Deprecated. Storing this until we decide to trash this entire folder.
...
class IntervalTree:
    def __init__(self, node, id, syntax):
        self.begin = node.meta.start_pos
        self.end = node.meta.end_pos
        self.id = id
        self.syntax = syntax
        self.next = None
        self.child = None
...
def build(tree, env, parser):
    intervals = []

    def _build(node):
        if isinstance(node, Token):
            return
        if isinstance(parser, LALRParser) and node.data in parser.module.syntatic_vars:
            var = node.children[0].value
            ftype, flbl = parser.module.ftable[var]
            intervals.append(IntervalTree(node, len(intervals), typemap[ftype]))
            intervals[-1].next = len(intervals)
            return
        elif (
            isinstance(parser, EarleyParser) or isinstance(parser, Grammar)
        ) and node.data in {"wff_var", "class_var", "setvar_var",}:
            intervals.append(
                IntervalTree(
                    node,
                    len(intervals),
...
def find(theorems, target, types_list, env, parser):
    ...
    try:
        ...
        if hasattr(parse_tree_target, "parser"):
            ta_intervals = build(
                parse_tree_target, env, parser.parsers[parse_tree_target.parser]
            )
        else:
            ta_intervals = build(parse_tree_target, env, parser)
    except LarkError:
        raise TargetError
    results = []
    for theorem, types in zip(theorems, types_list):
        ...
        try:
            ...
            if hasattr(parse_tree_target, "parser"):
                th_intervals = build(
                    parse_tree_theorem, env, parser.parsers[parse_tree_theorem.parser]
                )
            else:
                th_intervals = build(parse_tree_theorem, env, parser)
...
class SubstFinder:
    ...
    def process(self, goal: str, labels: List[str]):
        """
...
def print_interval_tree(expr, intervals, cur, depth=0):
    tabs = " " * depth
    print(tabs + expr[cur.begin : cur.end])
    if cur.child is not None:
        print_interval_tree(expr, intervals, intervals[cur.child], depth + 1)
    if cur.next is not None:
        print_interval_tree(expr, intervals, intervals[cur.next], depth)


def print_parse_tree(cur, depth=0):
    tabs = " " * depth
    print(tabs + cur.data)
    for c in cur.children:
        print_parse_tree(c, depth + 1)
...
if __name__ == "__main__":
    ...
    true_parser = Parser(grammar="holophrasm", earley_type="cpp")
    true_subst_finder = SubstFinder(mm_env, true_parser)
    ...
    for i, (label, tree) in enumerate(proof_trees.items()):
        ...
        for node in enumerate_nodes(
            ...
            try:
                start = time.time()
                found = true_subst_finder.get_subs(" ".join(statement), node.label)
            ...
            if found is None or not all(
                ...
                print(types)
                print(found)
                print(true_subst_finder.get_subs(" ".join(statement), node.label))
                assert False

...
class StructuredGoal:
    ...
    def to_expression(self) -> Dict[str, Union[str, int]]:
        ...
        for hyp in self.hyps:
            ...
            if to_add.startswith("∀ _inst") or any(ip in to_add for ip in instance_parameters):
    ...
    def to_command(self) -> Dict[str, Union[str, int]]:
        command = " ".join(h.to_command() for h in self.hyps) + " : " + self.conclusion.replace('\n', ' ')
...
def parse_goal_structured(goal_pp: str) -> List[StructuredGoal]:
    ...
    goal_pp = "\n".join(
        line for line in goal_pp.split("\n")
    ...
    while line_id < len(lines):
        ...
        while line_id < len(lines):
            ...
            if line.startswith("⊢"):
                ...
                this_hyp = line.strip()
                line_id += 1
                while line_id < len(lines) and (len(lines[line_id]) == 0 or lines[line_id][0] == ' '):
